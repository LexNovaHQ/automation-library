# Lex Nova HQ â€” Automation Library Component Catalog
...
**Discipline rule:** every reusable pattern is captured here as a component. Templates and client deliveries do NOT contain bespoke logic â€” they assemble pre-built components from this catalog. New patterns get added as components first, then assembled.

---

## Status Legend

- âœ… **Done** â€” fully built, tested, documented, ready to assemble
- ðŸŸ¡ **Partial** â€” built for a specific context, needs generalization to be assembly-ready
- âŒ **Not done** â€” design locked, build pending
...
- **ID** â€” `Cn-Letter` format (e.g., `C2-A`)
- **Name** â€” terse descriptor
- **Description** â€” what it does, 1-2 sentences
- **Category** â€” 1 through 6
- **Status** â€” Done / Partial / Not done
- **Backing offering(s)** â€” which catalogue offering(s) it powers
- **Backing template(s)** â€” which of the 5 demand-backed templates use it
- **Dependencies** â€” other components it relies on
- **Borrowed by** â€” which other categories borrow this component
- **Last tested** â€” date last validated end-to-end
- **Location in repo** â€” path under `components/`

---

## Status Summary

| Category | Total | âœ… Done | ðŸŸ¡ Partial | âŒ Not done |
|---|---|---|---|---|
...
# Category 1 â€” Cold Outbound Infrastructure

**Powers:** Catalogue Offering #1 â€” Cold Outbound Infrastructure ($497-$1,247)  
**Backing portfolio item:** Hunter Engine (repackaged)

| ID | Component | Description | Status |
|---|---|---|---|
| **C1-A** | Deliverability Stack Configurator | SPF/DKIM/DMARC + domain warming protocol + inbox rotation | âœ… Done |
| **C1-B** | Send Platform Deployer | Instantly / Smartlead / GMass setup as repeatable build pattern | ðŸŸ¡ Partial (GMass done) |
| **C1-C** | Reply Detection & Routing Engine | Classify replies (positive/OOO/negative/unsubscribe) and route | ðŸŸ¡ Partial |
| **C1-D** | AI Personalization Engine | LLM-driven dynamic copy referencing company-level intent data | âœ… Done |
| **C1-E** | Multi-Step Sequence Engine | Branching, delays, A/B logic for cold sequences | ðŸŸ¡ Partial |

**Borrowed from other categories:** C2-A, C2-B, C2-C, C2-D, C4-L, C5-E (KPI dashboard for sequence metrics)

---

# Category 2 â€” Custom Automation Workflows (Make.com / n8n)

**Powers:** Catalogue Offering #2 â€” Custom Automation Workflows ($147 single workflow / $497 multi-step)  
**Backing portfolio item:** TBD (built during Phase 2)

| ID | Component | Description | Status |
|---|---|---|---|
| **C2-A** | Data Sync Pipeline | Bidirectional sync between any two apps (CRMâ†”Sheets, Airtableâ†”Notion, etc.) | âœ… Done |
| **C2-B** | Conditional Routing Engine | If/then/else logic for routing records, leads, events | âœ… Done |
| **C2-C** | Webhook Trigger System | Event in App A triggers action in App B via webhook | âœ… Done |
| **C2-D** | Deduplication & Merge Engine | Collapse duplicate records across systems | ðŸŸ¡ Partial |
| **C2-E** | Form Intake Pipeline | Form submission â†’ enrichment â†’ CRM â†’ notification | âœ… Done |
| **C2-F** | Conditional Form Logic | Branching forms, qualifying paths | âœ… Done |
| **C2-G** | File Upload Routing | Form upload â†’ cloud storage â†’ metadata to CRM | ðŸŸ¡ Partial |
| **C2-H** | Payment-on-Intake Flow | Qualify â†’ payment link â†’ CRM update | âœ… Done |
| **C2-I** | Notification & Alert Engine | Slack/email/SMS triggers from app events with conditional logic | âœ… Done |
| **C2-J** | Digest/Summary Notification Builder | Scheduled rollups (daily/weekly) | âŒ Not done |
| **C2-K** | LLM-in-Workflow Adapter | Call Claude/GPT inside Make.com/n8n for transformation | âœ… Done |
| **C2-L** | Calendar/Scheduling Automation | Booking, availability, reminders, round-robin | âŒ Not done |
| **C2-M** | **WhatsApp Integration Adapter** *(NEW per D-PHASE2-005)* | WhatsApp Business API / Twilio / WATI: webhook receive, template send, media handling, session routing | âŒ Not done |
| **C2-N** | **Spreadsheet/CSV Parser** *(NEW per D-PHASE2-005)* | In-workflow CSV/Excel parsing for line items, bulk lists, bank statements (distinct from C5-S which is dashboard UI upload) | ðŸŸ¡ Partial |
...
## C2-C Build Record â€” Webhook Trigger System
...
## C2-I Build Record - Notification / Alert Engine
...
**Implementation note:**  
C2-I is built as the notification and alert engine. The current implementation is the email adapter inside the C2-I Notification & Alert Engine family. It accepts standardized upstream component output, sends an email alert only when notification is requested, and returns a standard notification result object.
...
# Category 3 â€” Client Onboarding System (GHL + Make.com)

**Powers:** Catalogue Offering #3 â€” Client Onboarding System ($497-$1,500)  
**Backing portfolio item:** GHL Onboarding Build + Client Intake Portal (anonymized Lex Nova client portal)

| ID | Component | Description | Status |
|---|---|---|---|
| **C3-A** | GHL Pipeline & Workflow Configurator | Custom GHL stages, fields, opportunity routing | âŒ Not done (GHL ramp pending) |
| **C3-B** | GHL Snapshot Builder | Reusable cloneable template across sub-accounts | âŒ Not done |
| **C3-C** | GHL â†” Make.com Bridge | Connect GHL native to external Make.com automations | ðŸŸ¡ Partial |
| **C3-D** | E-Signature Integration | DocuSign / HelloSign / native GHL signing embedded in flow | âŒ Not done |
| **C3-E** | Document Generation Pipeline | Data â†’ template â†’ PDF/Doc â†’ delivery (engagement letters, proposals) | âœ… Done |
| **C3-F** | Document Delivery & Tracking | Sent â†’ opened â†’ signed â†’ received status sync | âœ… Done |
| **C3-G** | Subscription Billing Engine | Recurring payments, plan tiers, dunning logic | ðŸŸ¡ Partial |
| **C3-H** | Payment-to-Onboarding Bridge | Payment confirmed â†’ trigger onboarding + portal creation | âœ… Done |
| **C3-I** | Welcome Sequence Engine (multi-path) | Different sequences for different client tiers/services | ðŸŸ¡ Partial |
| **C3-J** | SMS Reminder Cascade | Twilio-based SMS layer alongside email | âŒ Not done |
| **C3-K** | Onboarding Funnel Dashboard | Drop-off by stage, time-to-onboard, conversion metrics | ðŸŸ¡ Partial |
| **C3-L** | Client Health Scoring | Active vs. stalled vs. completed onboarding classification | âŒ Not done |
| **C3-M** | Proposal/SOW Auto-Generator | Qualified lead â†’ personalized proposal/SOW | ðŸŸ¡ Partial |
...
# Category 4 â€” AI Agents & RAG Systems

**Powers:** Catalogue Offering #4 â€” AI Agents & RAG Systems ($497-$2,500)  
**Backing portfolio item:** Copywriter (Architect â†’ Claude) + AI Contract Q&A System + Hunter Engine + AI Lead Qualification Agent

| ID | Component | Description | Status |
|---|---|---|---|
| **C4-A** | AI Classification Agent | Input â†’ category â†’ routing | âœ… Done |
| **C4-B** | AI Extraction Agent (text-based) | Unstructured text input â†’ structured output (JSON/fields). Excludes image OCR (see C4-T) | âœ… Done |
| **C4-C** | AI Summarization Agent | Long text â†’ controllable-length summary | âœ… Done |
| **C4-D** | AI Content Generation Pipeline | Template + variables + LLM â†’ output at scale | âœ… Done |
| **C4-E** | Multi-Agent Pipeline / Orchestration | Agent 1 â†’ Agent 2 â†’ Agent 3 with deterministic translation between | âœ… Done |
| **C4-F** | Document Ingestion Pipeline (RAG) | PDFs/docs â†’ chunking â†’ embedding â†’ vector store | ðŸŸ¡ Partial |
| **C4-G** | Semantic Retrieval Engine | Query â†’ top-K + reranking + filtering | ðŸŸ¡ Partial |
| **C4-H** | Q&A Interface with Source Citations | Query â†’ retrieval â†’ LLM answer with citations | ðŸŸ¡ Partial |
| **C4-I** | Knowledge Base Auto-Updater | Google Drive/Notion/Confluence â†’ vector store sync | ðŸŸ¡ Partial |
| **C4-J** | Domain-Tuned RAG | Legal/medical/accounting domain prompts + retrieval filters | âœ… Done |
| **C4-K** | Email-Triage Agent | Incoming emails â†’ categorize â†’ route â†’ suggest response | ðŸŸ¡ Partial |
| **C4-L** | Lead Qualification Agent | Inbound form/lead â†’ score â†’ enrich â†’ categorize | âœ… Done |
| **C4-M** | Agent Memory Layer | Persistent conversation history, context recall | ðŸŸ¡ Partial |
| **C4-N** | Tool-Use Agent | LLM calls APIs/databases, takes real-world actions | ðŸŸ¡ Partial |
| **C4-O** | Human-in-the-Loop Checkpoint | Agent proposes â†’ human approves â†’ action executes | âŒ Not done |
| **C4-P** | Internal AI Copilot / Knowledge Assistant | Team-facing AI trained on company SOPs | ðŸŸ¡ Partial |
| **C4-Q** | Prompt Template Library | Curated prompts for specific tasks, team-facing | âŒ Not done |
| **C4-R** | AI Output QA / Filtering Layer | Quality check â†’ human review if low confidence | âŒ Not done |
| **C4-S** | AI Content Variation A/B Testing | Multiple outputs per input â†’ compare | âŒ Not done |
| **C4-T** | **Document OCR Pipeline (image-based)** *(NEW per D-PHASE2-005)* | Image upload â†’ OCR (Tesseract / Google Document AI / AWS Textract) â†’ text â†’ handoff to C4-B for structured extraction | ðŸŸ¡ Partial |

**Borrowed from other categories:** C2-A (sync), C2-C (webhook), C2-I (notifications), C5-E (KPI dashboard)

**Deferred (out of catalogue scope):** ~~C4-U Inbound Voice Agent~~, ~~C4-V Outbound Voice Agent~~, ~~C4-W Voice Transcript-to-Action~~ (Voice AI per D63 â€” revisit Month 3+)

---

# Category 5 â€” Custom Internal Dashboards & Client Portals

**Powers:** Catalogue Offering #5 â€” Custom Dashboards & Client Portals ($797-$3,000)  
...
| ID | Component | Description | Status |
|---|---|---|---|
| **C5-A** | Authentication & Session Engine | Email/password, magic-link, OAuth, session management, password reset | âœ… Done (magic-link) / ðŸŸ¡ (full session UX) |
| **C5-B** | Magic-Link Portal Engine | Passwordless client portal access | âœ… Done |
| **C5-C** | Client Document Vault | Per-client folder, version history, access controls | âœ… Done |
| **C5-D** | Project Status Visibility | Stage tracker, timeline, deliverable preview | âœ… Done |
| **C5-E** | KPI Dashboard | Configurable metrics, time-window selectors, drill-down | âœ… Done |
| **C5-F** | Multi-Tenant Admin Panel | Superadmin view across all clients, RBAC-ready | âœ… Done |
| **C5-G** | CRUD Table Builder | Add/edit/delete records with validation | âœ… Done |
| **C5-H** | Filterable Data Grid | Search, sort, filter, paginate across thousands of rows | âœ… Done |
| **C5-I** | Pipeline/Funnel View | Drag-and-drop or list-based stage progression | ðŸŸ¡ Partial |
| **C5-J** | Real-Time Notification Center | In-app notifications with read/unread, action links | ðŸŸ¡ Partial |
| **C5-K** | Calendar Embed | Native calendar view inside dashboard | âŒ Not done |
| **C5-L** | File Upload + Preview | Drag-drop upload, in-browser preview for PDFs/images | ðŸŸ¡ Partial |
| **C5-M** | Form Builder (Admin-Side) | Admin builds intake forms; clients fill via portal | ðŸŸ¡ Partial |
| **C5-N** | Audit Log / Activity Stream | Who did what, when, with diffs | ðŸŸ¡ Partial |
| **C5-O** | Settings / Configuration Panel | Per-tenant config, feature flags, branding | âœ… Done |
| **C5-P** | Stripe/Razorpay Payment Embed | In-dashboard payment processing | ðŸŸ¡ Partial |
| **C5-Q** | Webhook Endpoint Builder | Client configures their own webhooks via UI | âŒ Not done |
| **C5-R** | Task/Checklist Engine | Assign tasks to user, due dates, status | âŒ Not done |
| **C5-S** | Data Import Flow (UI-based) | CSV/Excel upload via dashboard â†’ parse â†’ map â†’ import | ðŸŸ¡ Partial |
| **C5-T** | Data Export Flow | Generate CSV/Excel/PDF from dashboard data | ðŸŸ¡ Partial |
| **C5-U** | API Endpoint Layer | REST endpoints for client's external systems | âœ… Done |
| **C5-V** | Mobile-Responsive UI Pattern | Works on phone/tablet without native app | âœ… Done |

**Borrowed from other categories:** C2-A, C2-C, C2-I, C2-L, C4-D

**Out of scope:** Native mobile app (iOS/Android), Offline-first / PWA â€” outside solo stack

---

# Category 6 â€” Diagnostic & Repair (NEW per D-PHASE2-004)

**Powers:** Catalogue Offering #6 â€” Fix Broken Make.com / Zapier / n8n Automations ($97 diagnostic + $150-800 repair)  
**Backing portfolio item:** TBD â€” built after first 2-3 diagnostic engagements

| ID | Component | Description | Status |
|---|---|---|---|
| **C6-A** | Workflow Diagnostic Checklist (SOP) | 30-50 point checklist covering common failure modes: webhook timeouts, rate limits, schema mismatches, OAuth expiry, malformed payloads, missing error handlers, retry storms | âŒ Not done |
| **C6-B** | Error Pattern Library / Failure Mode Taxonomy | Catalog of common Make.com/Zapier/n8n failure patterns with root causes and fixes. Lives as Markdown reference in repo. Grows with every engagement. | âŒ Not done |
| **C6-C** | Webhook Health Tester | n8n workflow that pings client endpoints, captures response times, status codes, timeouts. Output: structured health report. | âŒ Not done |
| **C6-D** | Schema/Payload Validator | Validates JSON payloads flowing through client's broken workflows against expected schemas. Catches malformed data â€” the #1 silent killer. | âŒ Not done |
| **C6-E** | Diagnostic Report Template | Standardized output format: 1-page summary + detailed findings + recommended repair scope + estimated repair cost. Anchors the upsell from $97 â†’ $200-800. | âŒ Not done |
| **C6-F** | Make-to-n8n Migration Toolkit | Common-case migration patterns (Make.com scenarios â†’ equivalent n8n workflows). Upsell engine: Offering #6 â†’ Offering #2. | âŒ Not done |
...
**The high-leverage components** (borrowed by 3+ other categories â€” these are your highest-priority builds):
...
## Template #1 â€” Lead Capture â†’ CRM â†’ AI Follow-up
...
## Template #2 â€” Client Onboarding System
...
## Template #3 â€” AI Document Processing
...
## Template #4 â€” Sales Pipeline Automation
...
## Template #5 â€” Invoice & Billing Automation
...
**Tier 1 â€” Build first (unlock multiple templates):**
1. C2-L Calendar/Scheduling Automation â€” used in Templates #2, #4; cross-borrowed by C3, C5
2. C2-M WhatsApp Integration Adapter â€” used in Templates #1, #2, #3; high market demand
3. C2-J Digest/Summary Notification Builder â€” cross-cutting

**Tier 2 â€” Build for specific template (Cat 2 sprint Days 2-10):**
4. C2-N Spreadsheet/CSV Parser (currently ðŸŸ¡, finish) â€” Templates #3, #5
5. C4-O Human-in-the-Loop Checkpoint â€” Template #3 (document approval), Offering #4
6. C4-Q Prompt Template Library â€” Offering #4 polish
7. C4-R AI Output QA / Filtering Layer â€” Template #3 quality gate
8. C4-S AI Content Variation A/B Testing â€” Offering #1 polish

**Tier 3 â€” Build for Offering #6 (after first 2-3 paid diagnostics):**
9. C6-A through C6-F (all six) â€” needed in priority order: C6-A â†’ C6-E â†’ C6-B â†’ C6-C â†’ C6-D â†’ C6-F

**Tier 4 â€” Build for Cat 3 (GHL ramp, on schedule):**
10. C3-A, C3-B, C3-D, C3-J, C3-L

**Tier 5 â€” Build for Cat 5 (Dashboards demand-led):**
11. C5-K, C5-Q, C5-R â€” build when a paid Offering #5 job demands them
...
| Voice AI (C4-U, C4-V, C4-W) | Deferred per D63 â€” revisit Month 3+ |
...
# Appendix A â€” Demo Toolkit Bindings (per Category)

**Purpose:** Each category's components bind to a specific demo toolkit. Portfolio templates inherit these bindings â€” they do NOT pick their own tools. This enforces the architectural rule that **tools bind to components, not to templates.** When a real paying client engages, components re-bind to the client's tools (their CRM, their form provider, their email service). Component logic stays identical â€” only the connection/auth config changes.
...
## A.1 â€” Category 2 Demo Toolkit v1.0 (locked per D-DAY2-003)
...
### A.1.1 â€” Role separation within the toolkit
...
C2-A (Data Sync Pipeline) supports all three (HubSpot, Airtable, Sheets) as destinations from Day 1 â€” implemented via n8n's native nodes for each.

---

### A.1.2 â€” Client-engagement re-binding
...
The C2-K LLM-in-Workflow Adapter is explicitly model-agnostic â€” uses n8n's OpenAI node with swappable base URL. Same logic, any provider.

---

### A.1.3 â€” Cat 2 demo account prerequisites

The accounts created ONCE for the entire Cat 2 sprint (no credit cards required):

1. Groq API key â€” console.groq.com
2. Tally account â€” tally.so
3. HubSpot free CRM â€” hubspot.com (real workspace, demo company)
4. Airtable workspace â€” airtable.com
5. Slack demo workspace â€” slack.com/create
6. Gmail App Password â€” myaccount.google.com â†’ Security (2FA required first)
7. Stripe test mode account â€” dashboard.stripe.com/register (switch Live â†’ Test in dashboard)
8. Calendly free account â€” calendly.com/signup (defer until Template #4)

Credentials stored locally in `D:\Code\lexnova-secrets.txt` â€” NEVER committed to GitHub.

---

## A.2 â€” Category 1 Demo Toolkit (TBD)
...
## A.3 â€” Category 3 Demo Toolkit (TBD)
...
## A.4 â€” Category 4 Demo Toolkit (TBD)
...
## A.5 â€” Category 5 Demo Toolkit (TBD)
...
## A.6 â€” Category 6 Demo Toolkit (TBD)
...
| v1.1 | 2026-05-24 | Added Appendix A â€” Demo Toolkit Bindings. Cat 2 toolkit locked per D-DAY2-003. Cat 1, 3, 4, 5, 6 toolkits marked TBD pending their sprints. |
